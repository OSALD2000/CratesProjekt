...
public class StacksCreater {
    public CrateStack[] stacks = new CrateStack[AoV_05.boxes[1].split(",").length ];

    public void stacksErsteller(){
        for(int i = 0; i<AoV_05.boxes[1].split(",").length ; i++){
            this.stacks[i] = new CrateStack(i+1);
        }
        ...
        for( String zeile : AoV_05.boxes){
            for(String spalteInhalt : zeile.split(",")){
                this.stacks[inxZeile].getStack()[inxStackZeile] = spalteInhalt;
                inxZeile++;
            }
            inxZeile=0;
            inxStackZeile--;
        ...
        for(CrateStack inhalt: this.stacks){
